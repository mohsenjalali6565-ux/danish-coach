...
export default function SpeakButton({ text, className = "", title = "Listen" }: Props) {
  ...
  return (
    <button
      ...
      title={title}
      type="button"
      className={`inline-flex items-center justify-center rounded-full w-6 h-6 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors flex-shrink-0 ${className}`}
    >
